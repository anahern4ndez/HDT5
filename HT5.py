...
def proceso(env, name, numero, arriving_time, computadora):
    ...
    print "Memoria otorgada al momento %s al proceso %s. Ha pasado al estado 'Ready' en %s." % (env.now, name, env.now)
    ...
    while not terminado:
        with computadora.CPU.request() as req:
            ...
            print "Espera al CPU el proceso %s con %d instrucciones en %s"% (name,cantidadMemoria,env.now)
            yield req
            if instrucciones>3:
...
compu = Computer(env)
for i in range(2):
    env.process(proceso(env,'%d'%i,i,random.expovariate(1.0/10), compu))
